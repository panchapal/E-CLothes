...
import logo from "../../Assets/Blue_White_Cute_Fashion_Logo-removebg-preview.png";
...
const navItems = [
  ...
  { name: "Add To Cart", route: "/createproduct" },
...
function Header() {
  ...
  const filteredNavItems = isLogin
    ? navItems // Show all nav items if logged in
    : navItems.filter(item => item.name !== "Add To Cart" && item.name !== "Product"); // Hide specific nav items if not logged in
                ...
                <>
                  <Tooltip title="Open settings">
                    <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                      <Avatar
                        alt="User Avatar"
                        src={userDetails?.profile_pic ? profile_Url(userDetails.profile_pic) : ''}
                      />
                    </IconButton>
                  </Tooltip>
                  <Menu
                  ...
                  <span style={{ marginRight: '10px', color: 'black' }}>Welcome, {isLogged}</span>
